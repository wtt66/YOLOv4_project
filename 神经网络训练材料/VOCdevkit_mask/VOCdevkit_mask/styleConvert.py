...
classes1 = ["nomask", "mask"]
classes2 = ["no_mask", "have_mask"]
...
def convert_annotation(image_id):
    ...
    for obj in root.iter('object'):
        ...
        if name in classes1:
            cls_id = classes1.index(name)
        elif name in classes2:
            cls_id = classes2.index(name)
        else:
            raise Exception(in_file.name+" have error value of `name` node")
...
train_file.close()
test_file.close()
VOC_train_file = open(os.path.join(
    work_sapce_dir, "ImageSets\\Main\\train.txt"), 'w')
VOC_test_file = open(os.path.join(
    work_sapce_dir, "ImageSets\\Main\\test.txt"), 'w')
...
VOC_train_file = open(os.path.join(
    work_sapce_dir, "ImageSets\\Main\\train.txt"), 'a')
VOC_test_file = open(os.path.join(
    work_sapce_dir, "ImageSets\\Main\\test.txt"), 'a')
